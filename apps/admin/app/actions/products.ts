...
import { requireAdminRole, requireStaffRole } from "@xuanvu/shared/auth/role";
import { createSupabaseAdminClient } from "@xuanvu/shared/supabase/admin";

export type ProductActionError = {
  code: "FORBIDDEN" | "UNAUTHENTICATED" | "SYSTEM_ERROR" | "CONFLICT" | "NOT_FOUND" | "VALIDATION_ERROR";
  message: string;
};
...
function validationError(message: string): ProductActionResult<never> {
  return { ok: false, error: { code: "VALIDATION_ERROR", message } };
}

function mapStaffGuardError(
...
export async function listProducts(): Promise<ProductActionResult<ProductSummary[]>> {
  ...
  try {
    ...
    const products = (data as unknown as ProductSummaryRow[]).map((row) => ({
      id: row.id,
      name: row.name,
      slug: row.slug,
...
export async function updateProductPricing(
  ...
  const guardError = mapStaffGuardError(guard);
  if (guardError) return guardError;

  if (!id) return notFoundError("Sản phẩm không tồn tại.");

  const selling = input.priceSellingVnd;
  const compare = input.priceCompareAtVnd;

  if (compare != null && selling != null && compare < selling) {
    return validationError("Giá so sánh phải lớn hơn hoặc bằng giá bán.");
  }

  if (compare != null && selling == null) {
    return validationError("Cần có giá bán khi đặt giá so sánh.");
  }
  ...
  try {
    ...
    const { data, error } = await adminClient
      .from("catalog_products")
      .update({
        price_selling_vnd: selling,
        price_compare_at_vnd: compare,
        updated_at: new Date().toISOString(),
      })
      .eq("id", id)
      .select("id")
  ...
  try {
    ...
    if (isPublished) {
      const { data: row, error: selErr } = await adminClient
        .from("catalog_products")
        .select("id, price_selling_vnd, first_published_at")
        .eq("id", id)
        .maybeSingle<{
          id: string;
      ...
      if (selErr) return systemError();
      if (!row) return notFoundError("Sản phẩm không tồn tại.");
      if (row.price_selling_vnd == null) {
        return validationError("Cần có giá bán trước khi đăng sản phẩm.");
      }
